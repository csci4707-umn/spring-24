...
BufferDesc *StrategyGetBuffer(BufferAccessStrategy strategy, uint32 *buf_state,
  ...
  *from_ring = false;
  ...
  // changes,
  // 		 * so all the buffers are pinned (or were when we looked at
  // them).
...
int StrategySyncStart(uint32 *complete_passes, uint32 *num_buf_alloc) {
  ...
  if (num_buf_alloc) {
    *num_buf_alloc =
        pg_atomic_exchange_u32(&StrategyControl->numBufferAllocs, 0);
  }
  SpinLockRelease(&StrategyControl->buffer_strategy_lock);
  return result;
}
...
void StrategyInitialize(bool init) {
   ...
   */
  StrategyControl = (BufferStrategyControl *)ShmemInitStruct(
      "Buffer Strategy Status", sizeof(BufferStrategyControl), &found);
...
BufferAccessStrategy GetAccessStrategyWithSize(BufferAccessStrategyType btype,
  ...
  strategy = (BufferAccessStrategy)palloc0(
